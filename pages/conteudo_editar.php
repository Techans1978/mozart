...
function token(){ if(empty($_SESSION['csrf_cnt_edit'])) $_SESSION['csrf_cnt_edit']=bin2hex(random_bytes(16)); return $_SESSION['csrf_cnt_edit']; }
function check_token($t){ return !empty($_SESSION['csrf_cnt_edit']) && hash_equals($_SESSION['csrf_cnt_edit'], (string)$t); }

$id = (int)($_GET['id'] ?? 0);
$erros=[]; $ok=null;

/* listas auxiliares */
$categorias=[]; if($r=$conn->query("SELECT id,titulo FROM content_categories WHERE published=1 ORDER BY titulo")) while($x=$r->fetch_assoc()) $categorias[]=$x;
$grupos=[];     if($r=$conn->query("SELECT id,nome FROM grupos ORDER BY nome")) while($x=$r->fetch_assoc()) $grupos[]=$x;
$perfis = ['bigboss','admin','gerente','usuario']; // ajuste conforme seu sistema
...
$sel_groups=[]; $sel_roles=[];

/* carregar se edição */
if($id>0){
  $st=$conn->prepare("SELECT * FROM content_articles WHERE id=? LIMIT 1");
  $st->bind_param('i',$id); $st->execute(); $res=$st->get_result(); $row=$res->fetch_assoc(); $st->close();
  if(!$row) die('Artigo não encontrado.');
  $art=array_merge($art,$row);
  $rg=$conn->query("SELECT group_id FROM content_article_groups WHERE article_id={$id}");
  while($x=$rg->fetch_assoc()) $sel_groups[]=(int)$x['group_id'];
  $rr=$conn->query("SELECT role FROM content_article_roles WHERE article_id={$id}");
...
if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  if(!$erros){
    if($id>0){
      $st=$conn->prepare("UPDATE content_articles SET
      ...
      $st=$conn->prepare("INSERT INTO content_articles
    ...
    if(!empty($ok)){
      // sincroniza grupos/perfis
      $conn->query("DELETE FROM content_article_groups WHERE article_id={$id}");
      if($sel_groups){
        $ins=$conn->prepare("INSERT INTO content_article_groups(article_id,group_id) VALUES(?,?)");
        foreach($sel_groups as $g){ $ins->bind_param('ii',$id,$g); $ins->execute(); }
        $ins->close();
      }
      $conn->query("DELETE FROM content_article_roles WHERE article_id={$id}");
      if($sel_roles){
        $ins=$conn->prepare("INSERT INTO content_article_roles(article_id,role) VALUES(?,?)");
        foreach($sel_roles as $r){ $ins->bind_param('is',$id,$r); $ins->execute(); }
        $ins->close();
      }
      $ok='Artigo salvo com sucesso.';
    }else{
...
<div id="page-wrapper"><div class="container-fluid">
  <div class="row"><div class="col-lg-12"><h1 class="page-header"><?= $id?'Editar Artigo':'Novo Artigo' ?></h1></div></div>

  <?php if($erros): ?>
    <div class="alert alert-danger">
      <strong>Verifique:</strong>
      <ul style="margin:0;padding-left:18px"><?php foreach($erros as $e):?><li><?=h($e)?></li><?php endforeach;?></ul>
    </div>
  <?php elseif($ok): ?>
  ...
  <form method="post">
      ...
      <div class="row">
        <div class="col-md-8">
          <div class="form-group">
            <label>Título</label>
            <input class="form-control" name="titulo" value="<?=h($art['titulo'])?>" required>
          </div>
          <div class="form-group">
            <label>Apelido (slug)</label>
            <input class="form-control" name="apelido" value="<?=h($art['apelido'])?>" required>
            <div class="smallmuted">use hífens, sem espaços (ex.: meu-artigo-top)</div>
          </div>
        </div>
        <div class="col-md-4">
          <div class="form-group">
            ...
            <select class="form-control" name="status">
              <?php foreach(['publicado'=>'Publicado','nao_publicado'=>'Não Publicado','arquivado'=>'Arquivado','lixeira'=>'No Lixo'] as $k=>$v):?>
    ...
    <div class="card-pane">
      <!-- Intro -->
        ...
        <div class="smallmuted">Opcional. Se vazio, o feed pode usar um resumo automático do conteúdo.</div>
        </div>
    ...
    <div class="card-pane">
      <div class="row">
        ...
        <div class="col-md-6">
          <h4>Publicação</h4>
          <div class="form-group"><label>Início da publicação</label><input class="form-control" type="datetime-local" name="publish_up" value="<?= $art['publish_up']?date('Y-m-d\TH:i', strtotime($art['publish_up'])):'' ?>"></div>
          <div class="form-group"><label>Término da publicação</label><input class="form-control" type="datetime-local" name="publish_down" value="<?= $art['publish_down']?date('Y-m-d\TH:i', strtotime($art['publish_down'])):'' ?>"></div>
          <div class="form-group">
            <label><input type="checkbox" name="featured" <?=$art['featured']?'checked':''?>> Destaque no Feed</label>
          </div>
          ...
          <div class="form-group">
            <label><input type="checkbox" name="acesso_publico" <?=$art['acesso_publico']?'checked':''?>> Acesso público (ignora restrições)</label>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="card-pane">
      <div class="row">
        <div class="col-md-6">
          ...
          <div class="smallmuted">Selecione um ou mais grupos que podem visualizar.</div>
        </div>
        <div class="col-md-6">
          <h4>Acesso por Perfis</h4>
          <select class="form-control" name="roles[]" multiple size="6">
            <?php foreach($perfis as $p): ?>
              <option value="<?=$p?>" <?= in_array($p,$sel_roles,true)?'selected':'' ?>><?=h(ucfirst($p))?></option>
            <?php endforeach;?>
          </select>
          <div class="smallmuted">Perfis equivalem ao campo <code>usuarios.nivel_acesso</code>.</div>
        </div>
      </div>
    </div>
    ...
    <div class="card-pane">
      <div class="row">
        <div class="col-md-6">
          <div class="form-group"><label>Nota (interna)</label><input class="form-control" name="nota_interna" value="<?=h($art['nota_interna'])?>"></div>
        </div>
        <div class="col-md-6">
          <div class="form-group"><label>Observação da Versão</label><input class="form-control" name="versao_obs" value="<?=h($art['versao_obs'])?>"></div>
        </div>
      </div>
    </div>

    <button class="btn btn-primary">Salvar</button>
    <a class="btn btn-default" href="<?= BASE_URL.'/pages/conteudo_listar.php'?>">Voltar</a>
  </form>
</div></div>
<?php include_once ROOT_PATH.'/system/includes/code_footer.php'; ?>

<!-- TinyMCE 6 (self-hosted) -->
