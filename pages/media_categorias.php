...
function tok(){ if(empty($_SESSION['csrf_media_cat'])) $_SESSION['csrf_media_cat']=bin2hex(random_bytes(16)); return $_SESSION['csrf_media_cat']; }
function chk($t){ return !empty($_SESSION['csrf_media_cat']) && hash_equals($_SESSION['csrf_media_cat'], (string)$t); }
  ...
  $csrf=$_POST['csrf']??''; if(!chk($csrf)){ $err='Token inválido.'; }
    ...
    if($acao==='salvar'){
      ...
      if($titulo===''||$apelido===''){ $err='Título e Apelido são obrigatórios.'; }
        ...
        if($id>0){
          $st=$conn->prepare("UPDATE media_categories SET titulo=?,apelido=?,parent_id=?,published=?,ordering=?,descricao=?,updated_at=NOW(),updated_by=? WHERE id=? LIMIT 1");
          ...
          $st=$conn->prepare("INSERT INTO media_categories(titulo,apelido,parent_id,published,ordering,descricao,created_at,created_by) VALUES(?,?,?,?,?,?,NOW(),?)");
    ...
    if($acao==='excluir'){
      $id=(int)($_POST['id']??0);
      if($id>0){ $st=$conn->prepare("DELETE FROM media_categories WHERE id=? LIMIT 1"); $st->bind_param('i',$id); $ok=$st->execute(); $st->close(); if(!$ok) $err='Não foi possível excluir (possui filhos ou vínculos).'; }
    }
  }
}
...
$rows=[]; $res=$conn->query("SELECT c.id,c.titulo,c.apelido,c.parent_id,COALESCE(p.titulo,'-') AS pai,c.published,c.ordering,c.descricao
                             FROM media_categories c LEFT JOIN media_categories p ON p.id=c.parent_id
