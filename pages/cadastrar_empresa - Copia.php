...
$hasFornecedorTbl   = table_exists($dbc,'moz_fornecedor');   // Fornecedores
...
if ($_SERVER['REQUEST_METHOD']==='POST') {
  ...
  // Local (empresa) e Fornecedor (moz_fornecedor)
      ...
      <div class="grid cols-4">
        ...
        <div>
          ...
          <select name="cat_id" id="cat_id" required>
            <option value="">—</option>
            <?php if (!empty($cats_grouped)): ?>
              <?php foreach($cats_grouped as $grupo => $items): ?>
                <optgroup label="<?= h($grupo) ?>">
                  <?php foreach($items as $it): ?>
                    <option value="<?= (int)$it['id'] ?>" <?= $rec['cat_id']==$it['id']?'selected':'' ?>>
                      <?= h($it['label']) ?>
                    </option>
                  <?php endforeach; ?>
                </optgroup>
