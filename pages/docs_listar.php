...
function tok(){ if(empty($_SESSION['csrf_docs_list'])) $_SESSION['csrf_docs_list']=bin2hex(random_bytes(16)); return $_SESSION['csrf_docs_list']; }
function chk($t){ return !empty($_SESSION['csrf_docs_list']) && hash_equals($_SESSION['csrf_docs_list'], (string)$t); }
function redir(){ header('Location: '.strtok($_SERVER['REQUEST_URI'],'?').(empty($_GET)?'':'?'.http_build_query($_GET))); exit; }

$EDIT = BASE_URL.'/pages/docs_editar.php';
$CATS = BASE_URL.'/pages/docs_categorias.php';
...
if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  $sql=[
    'publicar'    => "UPDATE doc_items SET status='publicado' WHERE id=? LIMIT 1",
    'despublicar' => "UPDATE doc_items SET status='nao_publicado' WHERE id=? LIMIT 1",
    'arquivar'    => "UPDATE doc_items SET status='arquivado' WHERE id=? LIMIT 1",
    'lixeira'     => "UPDATE doc_items SET status='lixeira' WHERE id=? LIMIT 1",
  ][$acao] ?? null;
...
$w=[]; $p=[]; $t='';
if($q!==''){ $w[]="(i.titulo LIKE CONCAT('%',?,'%') OR i.apelido LIKE CONCAT('%',?,'%'))"; $p[]=$q; $p[]=$q; $t.='ss'; }
if($status!=='todos'){ $w[]="i.status=?"; $p[]=$status; $t.='s'; }
if($cat>0){ $w[]="i.categoria_id=?"; $p[]=$cat; $t.='i'; }

$sql="SELECT i.id,i.titulo,i.apelido,i.status,i.show_in_feed,i.publish_up,i.publish_down,c.titulo AS categoria
      FROM doc_items i LEFT JOIN doc_categories c ON c.id=i.categoria_id";
...
<div id="page-wrapper"><div class="container-fluid">
  <div class="row"><div class="col-lg-12"><h1 class="page-header">Documentos</h1></div></div>
  ...
  <form class="form-inline" method="get" style="margin-bottom:12px">
    ...
    <a class="btn btn-success" href="<?=$EDIT?>" style="margin-left:8px">➕ Novo Documento</a>
    <a class="btn btn-default" href="<?=$CATS?>" style="margin-left:8px">Categorias</a>
  </form>

  <div class="table-responsive">
    <table class="table table-striped table-hover">
      <thead><tr><th style="width:70px">ID</th><th>Título</th><th>Categoria</th><th>Feed</th><th>Status</th><th>Publicação</th><th style="width:260px">Ações</th></tr></thead>
      <tbody>
      <?php if(!$rows): ?>
