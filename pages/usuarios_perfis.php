...
$perfis = [];
$res = $conn->query("SELECT id, COALESCE(path_cache,nome) AS label FROM perfis ORDER BY depth ASC,path_cache ASC,nome ASC");
while($r=$res->fetch_assoc()) $perfis[]=$r;
...
<div id="page-wrapper">
  <div class="container-fluid">
    ...
    <h2>Perfis de <?= htmlspecialchars($u['nome_completo']) ?></h2>

    <form id="formAddPerfil" class="form-inline">
    ...
    <table id="tabelaPerfis" class="table table-bordered table-striped"></table>
  </div>
</div>
<?php include_once ROOT_PATH.'/system/includes/code_footer.php'; ?>
<script>
$(function(){
  const $t = $('#tabelaPerfis').DataTable({
    ajax:{ url:'<?= BASE_URL ?>/services/usuarios_perfis_list.php', data:{usuario_id:<?= $usuario_id ?>}},
    columns:[
      {data:'id',title:'ID'},
      {data:'perfil_label',title:'Perfil'},
  ...
  $('#formAddPerfil').on('submit',function(e){
    e.preventDefault();
    $.post('<?= BASE_URL ?>/services/usuarios_perfis_add.php',$(this).serialize(),r=>{
      if(r.ok) $t.ajax.reload(); else alert(r.mensagem);
    },'json');
  });

  window.remover=id=>{
    if(confirm("Remover vínculo?")) $.post('<?= BASE_URL ?>/services/usuarios_perfis_delete.php',{id},r=>{
      if(r.ok) $t.ajax.reload(); else alert(r.mensagem);
    },'json');
  };
  window.toggle=(id,v)=>{
    $.post('<?= BASE_URL ?>/services/usuarios_perfis_toggle_primary.php',{id,valor:(v?0:1)},r=>{
      if(r.ok) $t.ajax.reload(); else alert(r.mensagem);
    },'json');
  };
});
