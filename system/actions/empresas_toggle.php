...
try {
  $stmt = $dbc->prepare("SELECT ativo FROM empresas WHERE id=? FOR UPDATE");
  $stmt->bind_param('i', $id);
  $stmt->execute();
  $res = $stmt->get_result()->fetch_assoc();
  $stmt->close();
  if (!$res) throw new Exception('Empresa não encontrada.');

  $novo = ($res['ativo'] ? 0 : 1);
  $stmt = $dbc->prepare("UPDATE empresas SET ativo=? WHERE id=?");
  $stmt->bind_param('ii', $novo, $id);
  if (!$stmt->execute()) throw new Exception($stmt->error);
  $stmt->close();

  $dbc->commit();
  echo json_encode(['mensagem'=> ($novo? 'Ativada' : 'Inativada') . ' com sucesso.']);
} catch (Exception $e) {
  $dbc->rollback();
