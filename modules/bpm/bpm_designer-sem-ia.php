...
<!-- Page Content -->
        ...
        <div class="shell">
          <!-- ===== MENU BAR (estilo desktop) ===== -->
          <div class="menubar">

            <!-- inputs escondidos (necessários para Abrir/Importar) -->
            ...
            <div class="menu">
              ...
              <div class="menu-dropdown">
                <button id="btnNormalizeIds">Normalizar IDs</button>
              </div>
            </div>

            <div class="menu">
              <button class="menu-btn">Importar ▾</button>
              <div class="menu-dropdown">
                <button id="btnAiCreatePrompt">Criar com IA…</button>
                <button id="btnAiCreateFull">Criar completo por IA</button>
                <button id="btnImportFluig">Importar Fluig (.bpmn/.json/.zip)</button>
                <button id="btnImportCamunda">Importar Camunda (.bpmn)</button>
              </div>
            </div>
            ...
            <div class="menu">
              ...
              <div class="menu-dropdown">
                <button id="btnExportXML">Baixar XML</button>
                <button id="btnExportSVG">Baixar SVG</button>
                <button id="btnExportJSON">Baixar JSON</button>
              </div>
            </div>

            <!-- mantém template exatamente como já existe -->
            <div class="menu template-box">
              <label for="tplSelect">Template</label>
              <select id="tplSelect"></select>
              <button id="btnApplyTpl">Aplicar</button>
            </div>
...
if (fileOpenEl) fileOpenEl.addEventListener('change', async (ev) => {
...
// Imports (MVP: aceita BPMN/XML; JSON/ZIP entram na Fase 7)
bind('btnImportFluig', () => $('#fileImportFluig')?.click());
bind('btnImportCamunda', () => $('#fileImportCamunda')?.click());

const fileFluigEl = $('#fileImportFluig');
if (fileFluigEl) fileFluigEl.addEventListener('change', async (ev) => {
  const f = ev.target.files?.[0];
  if (!f) return;
  ...
  if (name.endsWith('.bpmn') || name.endsWith('.xml')) {
    ...
    alert('Import Fluig: MVP aceita só .bpmn/.xml por enquanto (JSON/ZIP na Fase 7).');
  }
  ev.target.value = '';
});

const fileCamundaEl = $('#fileImportCamunda');
if (fileCamundaEl) fileCamundaEl.addEventListener('change', async (ev) => {
  const f = ev.target.files?.[0];
  if (!f) return;
...
// ===== IA (placeholders seguros) =====
      bind('btnAiCreatePrompt', () => {
        alert('Criar com IA: em construção');
      });

      bind('btnAiCreateFull', () => {
        alert('Criar completo por IA: em construção');
      });
