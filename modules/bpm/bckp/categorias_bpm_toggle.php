...
try {
  $stmt = $dbc->prepare("SELECT ativo FROM bpm_categorias WHERE id=? FOR UPDATE");
  $stmt->bind_param('i', $id);
  $stmt->execute();
  $r = $stmt->get_result()->fetch_assoc();
  $stmt->close();
  if (!$r) throw new Exception('Categoria não encontrada.');

  $novo = $r['ativo'] ? 0 : 1;
  $stmt = $dbc->prepare("UPDATE bpm_categorias SET ativo=? WHERE id=?");
