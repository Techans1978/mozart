...
var IdGenerator = require('diagram-js/lib/util/IdGenerator');
...
Minimap.prototype._updateElementId = function(element, newId) {
  ...
  try {
    var elementGfx = minDom.query('#' + EscapeUtil.escapeCSS(this._prefixId(element.id)), this._elementsGroup);
...
Minimap.prototype._addElement = function(element) {
  ...
  var newElementParentGfx = minDom.query('#' + EscapeUtil.escapeCSS(this._prefixId(element.parent.id)), this._elementsGroup);
...
Minimap.prototype._createElement = function(element) {
  ...
  if (gfx) {
    ...
    if (visual) {
      var elementGfx = sanitize(tinySvg.clone(visual));

      tinySvg.attr(elementGfx, { id: this._prefixId(element.id) });
...
function sanitize(gfx) {
  minDom.queryAll('[id]', gfx).forEach(function(element) {
    element.remove();
  });
