...
// secret da instância ou env ou default
...
$HEALTH = 'http://' . $WPP_HOST . ':' . $WPP_PORT . '/health';
...
$tok         = http_request_wpp_local('POST', $genTokenUrl, ['Content-Type'=>'application/json'], '{}', $TIMEOUT_S);
...
if ($token && ($_SERVER['REQUEST_METHOD']==='POST')) {
    ...
    if (isset($_POST['admin'])) {
        ...
        if ($act === 'status_session') {
            ...
            $adminResp = http_request_wpp_local('POST', "{$BASE}/{$s}/logout-session", ['Authorization'=>"Bearer {$token}", 'Content-Type'=>'application/json'], '{}', $TIMEOUT_S);
        } elseif ($act === 'close') {
            $adminResp = http_request_wpp_local('POST', "{$BASE}/{$s}/close-session", ['Authorization'=>"Bearer {$token}", 'Content-Type'=>'application/json'], '{}', $TIMEOUT_S);
        } elseif ($act === 'delete') {
...
<div class="container-fluid mt-3">
  <div class="wpp-card">
    ...
    <div class="wpp-kv">
      ...
      <div>Secret (fonte):</div><div class="wpp-small"><?= $SECRET_FROM_ENV!==false ? 'ENV WPP_SECRET' : 'moz_wpp_instance.secret / default' ?></div>
    </div>
    <p class="wpp-muted">Esta tela gera o token, inicia a sessão, verifica o status, exibe o QR quando necessário e permite testar envio/leitura para esta instância.</p>
  </div>
