...
$empresas = [];
if ($hasEmpresaTbl) {
  $sqlEmp = "SELECT id, COALESCE(NULLIF(nome_fantasia,''), nome_empresarial) AS nome
               FROM empresas ".(has_col($dbc,'empresas','ativo') ? "WHERE ativo=1" : "WHERE 1=1")."
              ORDER BY nome_empresarial ASC";
...
$fornecedores = [];
if ($hasFornecedorTbl) {
  $rs=$dbc->query("SELECT id,nome FROM moz_fornecedor ORDER BY nome");
  if ($rs) while($r=$rs->fetch_assoc()) $fornecedores[]=$r;
}
    ...
    try {
      if ($id>0) {
        ...
        $sql = "UPDATE moz_ativo SET
                  nome=?, cat_id=?, marca_id=?, modelo_id=?, tag_patrimonial=?, numero_serie=?, status_id=?, local_id=?, fornecedor_id=?, "
             . ($hasNF ? "nf_numero=?," : "")
             . " data_compra=?, "
             ...
             . " WHERE id=?";
        // mapa base: nome(s), cat(i), marca(i), modelo(i), tag(s), serie(s), status(i), local(i), fornecedor(i)
        $types = "siiissiii";
