...
// ========= Export CSV (lista completa) =========
// Evita conflito: a manutencao_lib.php pode já ter esses helpers
...
if (($_GET['export'] ?? '') === 'csv') {
  // CSRF simples via query (pra não deixar download aberto)
  ...
  // qual coluna de setor vamos usar no join (se existir)
  $sectorExpr = "NULL";
  ...
  $sql = "
    ...
    ORDER BY COALESCE(m.data_manutencao, m.data_prevista, m.created_at) DESC, m.id DESC
  ";
  ...
  while ($r = $rs->fetch_assoc()) {
    ...
    $prev = $r['data_prevista'] ? date('d/m/Y', strtotime($r['data_prevista'])) : '';
    $manu = $r['data_manutencao'] ? date('d/m/Y', strtotime($r['data_manutencao'])) : '';
  ...
  <section class="bpm"><div class="container">
    ...
    <header class="toolbar">
      ...
      <div class="actions">
        ...
        <a class="btn" href="manutencoes-listar.php?export=csv&csrf=<?= h($csrf) ?><?= $ativo_id?('&ativo_id='.(int)$ativo_id):'' ?>">Exportar CSV</a>
