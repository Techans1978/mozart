...
$empresas = [];
if ($hasEmpresaTbl) {
  // Nome Fantasia - Apelido (fallbacks se vazio)
  $sqlEmp = "
    SELECT id,
           TRIM(
             CONCAT(
...
/* ===== DEPOSITOS (tabela) =====
 * Ajuste $depositoTable se seu nome for outro (ex.: 'depositos')
 */
      ...
      <div class="grid cols-4">
        ...
        <div>
          ...
          <select name="manut_alertar" <?= $hasManutAlertar?'':'disabled' ?>>
            <option value="1" <?= ($rec['manut_alertar']??1)?'selected':'' ?>>Chamado Interno</option>
            <option value="0" <?= !($rec['manut_alertar']??1)?'selected':'' ?>>Chamado Glpi</option>
            <option value="0" <?= !($rec['manut_alertar']??1)?'selected':'' ?>>E-mail</option>
            <option value="0" <?= !($rec['manut_alertar']??1)?'selected':'' ?>>Whatsapp</option>
            <option value="0" <?= !($rec['manut_alertar']??1)?'selected':'' ?>>SMS</option>
          </select>
        </div>
      </div>
