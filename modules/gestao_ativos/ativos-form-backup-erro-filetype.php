...
function anexo_href(string $p): string
{
    ...
    // se já é URL absoluta, usa como está
    if (preg_match('#^https?://#i', $p)) return $p;

    // se já começa com "/" (raro no seu caso), normaliza
    if ($p[0] === '/') $p = ltrim($p, '/');
    return rtrim(BASE_URL, '/') . '/' . $p;
}

// Nome amigável para mostrar na lista // Se for download.php?id=..., mostra "Arquivo #ID" // Senão, mostra o nome do arquivo (basename)
...
function storage_save_upload(mysqli $dbc, string $module, int $entityId, array $file, array $allowExt): array
{
    ...
    if (table_exists($dbc, 'moz_file')) {
        ...
        if (has_col($dbc, 'moz_file', 'ext')) $add('ext', 's', $ext); // <- só entra se existir
...
Obs.: Atenção as listas ga-list quando tiver subcategorias em todos deve apresentar "Pai - Filho - Neto" e assim por diante.
*/
        ...
        SELECT
            ...
            TRIM(
                CONCAT(
                    ...
                    CASE WHEN COALESCE(NULLIF(apelido,''),'') <> '' THEN CONCAT(' - ', apelido) ELSE '' END
                )
            ) AS nome
        FROM empresas
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // uploads
    // uploads (NOVO: fora do public, padrão data_files/ativos/{id}/{timestamp_rand}/)
    $photos_paths  = [];
    $contrato_path = null;
    $outros_paths  = [];
    ...
    if (
        !$err &&
        !empty($_FILES['contrato_pdf']) &&
        ($_FILES['contrato_pdf']['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_NO_FILE
    ) {
    ...
    if (!$err) {
        ...
        try {
            if ($id > 0) {
                ...
                if ($col_cat_item) {
                    $types .= "i";
                    $args[] = $categoria_item_id;
                }
                if ($col_inf_cat) {
                    $types .= "i";
                    $args[] = $informar_categoria;
                }
                if ($col_nome_item) {
                    $types .= "i";
                    $args[] = $nome_item_id;
        ...
        <section class="bpm">
            <div class="container">
                ...
                <?php if (!empty($_SESSION['flash_ok'])): ?>
                    <div class="alert alert-success"><?= h($_SESSION['flash_ok']);
                ...
                <form class="card" method="post" enctype="multipart/form-data" autocomplete="off" novalidate>
                        ...
                        <div>
                            ...
                            <div style="display:flex; gap:10px; align-items:center; margin-bottom:6px;">
                                <label style="display:flex; gap:6px; align-items:center; font-weight:normal; margin:0;">
                                    <input type="checkbox" id="nome_digit" name="nome_digit" value="1" <?= !empty($rec['nome_digit']) ? 'checked' : '' ?>> Digitar
                                </label>
                    ...
                    <div class="grid cols-4" style="align-items:end">
                        <div>
                            <label style="display:flex; gap:6px; align-items:center; font-weight:normal; margin:0;">
                                <input type="checkbox" id="informar_categoria" name="informar_categoria" value="1" <?= !empty($rec['informar_categoria']) ? 'checked' : '' ?>> Informar categoria
                            </label>
                    ...
                    <div class="grid cols-4" style="align-items:end">
                        <div>
                            <label style="display:flex; gap:6px; align-items:center; font-weight:normal; margin:0;">
                                <input type="checkbox" id="informar_deposito" name="informar_deposito" value="1" <?= !empty($rec['informar_deposito']) ? 'checked' : '' ?>> Informar Depósito
                            </label>
                                ...
                                <div style="margin-bottom:8px;">
                                    ...
                                    <ul style="margin:6px 0 0 18px;">
                                        <?php foreach ($anexos['foto'] as $p): ?>
                                            <li>
                                                <a href="<?= h(anexo_href($p)) ?>" target="_blank" rel="noopener">baixar</a> — <?= h(anexo_label($p)) ?>
                                            </li>
                                        <?php endforeach; ?>
                                    </ul>
                                </div>
...
<script>
    ...
    $btnAddAtr?.addEventListener('click', () => {
        ...
        if (!esc) {
            alert('Selecione um item da lista ou informe o ID no formato #123.');
            return;
        }
