...
// garante UTF-8 nas funções multibyte
  ...
  if (empty($_FILES['csv']['tmp_name']) || !is_uploaded_file($_FILES['csv']['tmp_name'])) {
    ...
    if (($fh = fopen($tmpfile, 'r')) === false) {
      ...
      while (($r = fgetcsv($fh, 0, $delim)) !== false) {
        ...
        // converte toda a linha para UTF-8 (planilhas em ISO-8859-1 / Windows-1252)
        foreach ($r as &$v) {
  ...
  if (!$rows || !is_array($rows)) {
    ...
    if (!$st) {
      ...
      foreach ($rows as $idx=>$r) {
        ...
        // -------- Categoria (criação usa SEMPRE o texto da planilha) --------
        ...
        // -------- Marca (criação usa texto da planilha: marca_nome) --------
        ...
        if (!$marca_id && $marca_nome !== '') {
          ...
          if (!$marca_id) {
            if ($c = $dbc->prepare("INSERT INTO moz_marca (nome,ativo) VALUES (?,1)")) {
              $c->bind_param('s',$marca_nome);
              $c->execute();
              $marca_id = $c->insert_id;
              $c->close();
            }
          }
        }

        // -------- Modelo (criação usa texto da planilha: modelo) --------
        ...
        if (!$modelo_id && $modelo_nome !== '') {
          ...
          // se ainda não achou, cria com nome da planilha
          if (!$modelo_id && $modelo_nome !== '' && $cat_id) {
            if ($c = $dbc->prepare("INSERT INTO moz_modelo (cat_id,marca_id,nome,ativo) VALUES (?,?,?,1)")) {
              $mid = $marca_id ?: 0;
              $c->bind_param('iis',$cat_id,$mid,$modelo_nome);
              $c->execute();
              $modelo_id = $c->insert_id;
              $c->close();
            }
          }
        }
        ...
        // -------- Fornecedor (criação usa texto da planilha: fornecedor_nome) --------
        ...
        if (!$fornecedor_id && $fornecedor_nome !== '' && $hasFornecedorTbl) {
          ...
          if (!$fornecedor_id) {
            if ($c = $dbc->prepare("INSERT INTO moz_fornecedor (nome,ativo) VALUES (?,1)")) {
              $c->bind_param('s',$fornecedor_nome);
              $c->execute();
              $fornecedor_id = $c->insert_id;
              $c->close();
            }
          }
        }
    ...
    <?php elseif ($step === 'preview' && $previewRows): ?>
      ...
      <form class="card" method="post">
        ...
        <p class="hint">
          ...
          Se o combo ficar em “—”, na importação será usado o valor da planilha para criar
          categoria/marca/modelo/fornecedor, se ainda não existirem.
        </p>
